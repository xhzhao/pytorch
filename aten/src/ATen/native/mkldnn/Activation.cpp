...
Tensor mkldnn_relu(const Tensor& input, double negative_slope) {
  AT_ERROR("mkldnn_relu: ATen not compiled with MKLDNN support");
}

Tensor& mkldnn_relu_(Tensor& input, double negative_slope) {
...
Tensor mkldnn_relu(const Tensor& input, const double negative_slope = 0.0f) {
...
Tensor& mkldnn_relu_(Tensor& input, const double negative_slope = 0.0f) {
  ...
  if (output_memory != input_usr_memory)
    MKLDNN_EXEC(reorder(output_memory, input_usr_memory));

  return input;
}

Tensor mkldnn_relu_backward(const Tensor& input, const Tensor& grad_output_t,
    const double negative_slope = 0.0f) {

  auto grad_output = grad_output_t.contiguous();
