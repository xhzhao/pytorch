...
std::tuple<at::Tensor, at::Tensor> mkldnn_pooling(const Tensor& input, IntList kernel_size,
    IntList stride, IntList padding, bool ceil_mode) {
...
at::Tensor mkldnn_pooling_backward(const Tensor& input, const Tensor& grad_output_t, const Tensor& indice,
    IntList kernel_size, IntList stride, IntList padding, bool ceil_mode) {
...
std::vector<int64_t> pooling_output_size(IntList input_size, IntList kernel_size, IntList stride,
     IntList padding, bool ceil_mode) {
...
std::tuple<at::Tensor, at::Tensor> mkldnn_pooling(const Tensor& input, IntList kernel_size,
  ...
  auto output_size = pooling_output_size(input_size, kernel_size, stride, padding, ceil_mode);
  ...
  auto pooling_forward_desc = pooling_forward::desc(prop_kind::forward, algorithm::pooling_max,
    input_md, output_md, stride_tz, kernel_tz, padding1_tz, padding2_tz, padding_kind::zero);
  auto pooling_forward_pd = pooling_forward::primitive_desc(pooling_forward_desc, cpu_engine);
  ...
  auto output_pd = pooling_forward_pd.dst_primitive_desc();
  ...
  auto pool_workspace_memory = memory(pooling_forward_pd.workspace_primitive_desc(), indice.data_ptr());

  auto pool_forward = pooling_forward(pooling_forward_pd, input_memory, output_memory, pool_workspace_memory);
  net.push_back(pool_forward);
...
at::Tensor mkldnn_pooling_backward(const Tensor& input, const Tensor& grad_output_t, const Tensor& indice,
  ...
  auto pool_pd = pooling_forward::primitive_desc(pooling_forward_desc, cpu_engine);
  auto pool_workspace_memory = memory(pool_pd.workspace_primitive_desc(), indice.data_ptr());

  auto pool_diff_dst_memory = memory({{{ output_tz }, data_t, format_input}, cpu_engine}, grad_output.data_ptr());

  auto pool_bwd_desc = pooling_backward::desc(pooling_max, pool_src_md, pool_dst_md,
    stride_tz, kernel_tz, padding1_tz, padding2_tz, padding_kind::zero);

  auto pool_bwd_pd = pooling_backward::primitive_desc(pool_bwd_desc, cpu_engine, pool_pd);
  ...
  auto pool_bwd = pooling_backward(pool_bwd_pd, pool_diff_dst_memory, pool_workspace_memory, pool_diff_src_memory);
